...
export default function RightPanel({ frame, hideInference = false }) {
  ...
  return (
    <aside style={T.panel}>
      ...
      <Section title="Thermal — Coolers">
        <MetricRow label="Oil Cooler In"       value={f1(row.oil_cooler_inlet_temp_c)}   unit="°C"  status={alertStatus('oil_cooler_inlet_temp_c')} />
        <MetricRow label="Oil Cooler Out"      value={f1(row.oil_cooler_outlet_temp_c)}  unit="°C"  status={alertStatus('oil_cooler_outlet_temp_c')} />
        <MetricRow label="Aftercooler In"      value={f1(row.aftercooler_inlet_temp_c)}  unit="°C"  status={alertStatus('aftercooler_inlet_temp_c')} />
        <MetricRow label="Aftercooler Out"     value={f1(row.aftercooler_outlet_temp_c)} unit="°C"  status={alertStatus('aftercooler_outlet_temp_c')} />
        <MetricRow label="Air Inlet"           value={f1(row.air_inlet_temp_c)}          unit="°C"  status="ok" />
      </Section>

      <Section title="Pressure & Electrical">
